...
def is_valid(enter_num, n):    # функция проверки корректности введенного пользователем значения
    return enter_num.isdigit() and 1 <= int(enter_num) <= n
def play_game():    # функция запуска игры
    ...
    while True:  # запускаем бесконечный цикл
        ...
        enter_num = input()
        count_attempts += 1
        if not is_valid(enter_num, n):  # с помощью функции is_valid проверяем, чтобы число было корректным
